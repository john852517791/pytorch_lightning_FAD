...
class asvspoof_dataModule(L.LightningDataModule):
        ...
        def setup(self, stage: str):
            ...
            if stage == "fit":
                ...
                label_dev, file_dev = genSpoof_list(
                    dir_meta=self.dev_protocols_file,
                    is_train=True,
                    is_eval=False)
                
                self.asvspoof19_val_set = Dataset_ASVspoof2019_train(
                    list_IDs=file_dev,
                    labels=label_dev,
        ...
        def train_dataloader(self):
            return DataLoader(self.asvspoof19_trn_set, batch_size=self.args.batch_size, shuffle=True,drop_last = True,num_workers=8)

        def val_dataloader(self):
            return DataLoader(self.asvspoof19_val_set, batch_size=self.args.batch_size, shuffle=False,drop_last = False,num_workers=8)            
